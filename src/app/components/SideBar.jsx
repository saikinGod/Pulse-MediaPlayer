        ...
        <div className="relative group">
          {isExpanded ? (
            <div className="relative">
              ...
              <input type="text" placeholder="Search..." className="w-full bg-[#252525] text-gray-200 text-sm rounded-md py-2.5 pl-10 pr-4 outline-none border border-transparent focus:border-red-500/50 focus:bg-[#2a2a2a] transition-all" />
            </div>
          ) : (
            <button onClick={() => setIsExpanded(true)} className="w-full flex justify-center py-2 text-gray-400 hover:text-red-500 transition-colors">
              <Search size={22} />
            </button>
          )}
        </div>
            ...
            <div key={item.name} className="relative group flex items-center">
              <button
                ...
                className={`
                  ...
                  ${isActive ? "text-white" : "text-gray-400 hover:bg-[#252525] hover:text-gray-200"}
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute left-0 top-0 bottom-0 w-full bg-[#252525] rounded-lg -z-10"
                    transition={{
                      type: "spring",
                      stiffness: 300,
                      damping: 30,
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 bg-red-600 rounded-r-full shadow-[0_0_8px_rgba(220,38,38,0.8)]"
                    transition={{
                      type: "spring",
                      stiffness: 300,
                      damping: 20,
                ...
                <item.icon
                  size={22}
                  className={`${isActive ? "text-red-500" : "group-hover:text-gray-200"} transition-colors duration-200 relative z-20`}
                />
                <span className={`whitespace-nowrap transition-opacity duration-300 relative z-20 ${isExpanded ? "opacity-100" : "opacity-0 w-0 hidden"}`}>
                  {item.name}
                </span>
              </button>
